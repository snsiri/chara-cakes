...
//   if (!token) {
//     setError('Please sign in to add a custom cake to your cart.');
//     return;
//   }
...
import React, { useState, useEffect } from 'react';
import api from '../api/axios';          // ← api instance sends Authorization header automatically
import { Form, Row, Col } from 'react-bootstrap';
import 'bootstrap/dist/css/bootstrap.min.css';
import './CustomizeItem.css';

const CustomizeItem = ({ customer }) => {
  const [customizeData, setCustomizeData] = useState({
    custom_layers: 2, custom_size: '', custom_shape: '',
    custom_bases: [], custom_filling: [], custom_frosting: null,
    custom_decorations: null, custom_price: 0, custom_specifications: '', ingredients: []
  ...
  const fetchAvailableOptions = async () => {
    try {
      setLoading(true);
      // Use api (not axios) so the token interceptor attaches Authorization header
      const res = await api.get(
        `/api/customizes/available-options?size=${customizeData.custom_size}&shape=${customizeData.custom_shape}`
      );
      if (res.data.status === 'Success') {
        const opts = res.data.data;
        setAvailableOptions({
          cake_base:   opts.cake_base   || [],
          filling:     opts.filling     || [],
          frosting:    opts.frosting    || [],
          decorations: opts.decorations || []
        });
        setError('');
      } else {
        setError(res.data.message || 'Failed to fetch options');
      }
    } catch (err) {
      // 401 just means guest — show a friendly nudge, not a scary error
      if (err.response?.status === 401) {
        setError('Please sign in to load cake options.');
      } else {
  ...
  const handleChange = (e) => {
    ...
    } else if (name === 'custom_size' || name === 'custom_shape') {
      setCustomizeData(prev => ({
        ...prev, [name]: value,
        custom_bases:       prev.custom_bases.map(() => null),
        custom_filling:     prev.custom_filling.map(() => null),
  ...
  const handleFrostingSelect   = (id) => { const opt = availableOptions.frosting.find(o => o._id === id);    setCustomizeData(p => ({ ...p, custom_frosting:    opt || null })); };
  const handleDecorationSelect = (id) => { const opt = availableOptions.decorations.find(o => o._id === id); setCustomizeData(p => ({ ...p, custom_decorations: opt || null })); };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!localStorage.getItem('customerToken')) {
      setError('Please sign in to save your custom cake and add it to cart.');
      return;
    }
    if (!validateForm()) return;
    setLoading(true); setError(''); setSuccess('');
    try {
      const custRes = await api.post('/api/customizes/add', customizeData);
      if (custRes.data.status === 'Success') {
        const cartRes = await api.post('/api/cart/add-customize', {
          customize_id: custRes.data.data._id, quantity: 1
        });
        if (cartRes.data.status === 'Success') {
          setSuccess('Your custom cake has been added to cart! 🎂');
  ...
  return (
    <div className="customize-container1">
      ...
      <div className="customize-form-wrap">
        ...
        {!isLoggedIn && (
          <div style={{
            background:'#fff8f0', border:'1px solid #f5d0a0', borderRadius:12,
            padding:'14px 20px', marginBottom:20,
            fontFamily:'Quicksand,sans-serif', fontSize:'0.88rem',
            fontWeight:600, color:'#7a4a00', display:'flex', alignItems:'center', gap:10
          }}>
            <span>ℹ️</span>
            You're browsing as a guest.&nbsp;
            <a href="/login" style={{color:'#c85050', fontWeight:700}}>Sign in</a>
            &nbsp;to load options and add your cake to cart.
          </div>
        )}
